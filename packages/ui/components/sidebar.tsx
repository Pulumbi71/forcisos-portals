...
export function Sidebar({ items, logo, title }: SidebarProps) {
  ...
  return (
    <>
      <button
        className="fixed top-4 left-4 z-50 md:hidden p-2 rounded-lg bg-navy text-white"
      ...
      <div
        className={clsx(
          'fixed inset-0 bg-black/50 md:hidden transition-opacity z-40',
      ...
      <aside
        className={clsx(
          'fixed left-0 top-0 h-screen w-64 bg-navy text-white flex flex-col overflow-y-auto transition-transform md:translate-x-0 z-40',
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2">
          {items.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className="bg-cta-red text-white text-xs px-2 py-1 rounded-full">
                  {item.badge}
                </span>
              )}
            </Link>
        ...
        <div className="p-4 border-t border-teal/20">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-2 rounded-lg text-gray-300 hover:bg-cta-red hover:text-white transition-colors"
          >
            <LogOut size={20} />
            Sign Out
          </button>
